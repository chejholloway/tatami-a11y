...
export class CommandPalette {
  ...
  constructor(options: CommandPaletteOptions) {
    ...
    this.filter       = options.filter ?? ((item, q) => {
  ...
  private groupItems(items: CommandItem[]): Array<{ groupLabel: string | null; items: CommandItem[] }> {
  ...
  private handleInput(): void {
    ...
    this.filteredItems = query.length > 0
      ? this.commands.filter(c => this.filter(c, query))
      : this.commands;
  ...
  private handleInputKeyDown(e: KeyboardEvent): void {
    switch (e.key) {
      case 'ArrowDown':
        e.preventDefault();
        this.setActiveIndex(
          this.activeIndex < this.totalOptions - 1 ? this.activeIndex + 1 : 0
        );
        break;
      ...
      case 'ArrowUp':
        e.preventDefault();
        this.setActiveIndex(
          this.activeIndex > 0 ? this.activeIndex - 1 : this.totalOptions - 1
        );
        break;
