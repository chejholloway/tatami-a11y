...
export class DatePicker {
  ...
  private renderMonth(): void {
    ...
    const dayRows = Array.from(this.calendarGrid.querySelectorAll('[role="row"]:not([data-header])'));
    ...
    const cells: Array<Date | null> = [
      ...
      ...Array.from({ length: daysInMonth }, (_, i) => new Date(this.viewYear, this.viewMonth, i + 1)),
    ];
  ...
  private showDialog(): void {
    ...
    if (!reduced) {
      ...
      requestAnimationFrame(() => { this.dialog.style.opacity = '1'; });
    }
  }
